...
import {
  ...
  RefreshCw, ShieldAlert, Sparkles, Building2, Gauge, Clock, Search, X
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const handleToggleBusStatus = async (bus) => {
    const newStatus = bus.status === 'LIVE' || bus.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE';
    await api.put(`/api/buses/${bus.id}`, { status: newStatus });
    loadAllData();
  };
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-900/90 backdrop-blur-xl p-6 rounded-3xl border border-slate-800 shadow-xl">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400 font-medium">• Live Telemetry Dispatch</span>
            </div>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8">
            ...
            {/* Quick Fleet Dispatch Table */}
            <div className="bg-slate-900/90 rounded-3xl p-6 border border-slate-800 shadow-xl space-y-4">
              <h3 className="font-extrabold text-lg text-white">Transit Dispatch Operational Summary</h3>
                ...
                {buses.map((bus) => (
                  <div key={bus.id} className="p-4 rounded-2xl border border-slate-800 bg-slate-950 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-xl text-xs font-bold text-white bg-slate-800 hover:bg-slate-700 border border-slate-700 transition-colors"
                    >
                      {bus.status === 'LIVE' || bus.status === 'ACTIVE' ? 'Decommission' : 'Deploy Dispatch'}
                    </button>
                  </div>
                ))}
              </div>
              ...
              {buses.map((bus) => (
                <div key={bus.id} className="bg-slate-900/90 rounded-3xl p-6 border border-slate-800 shadow-xl space-y-4">
                  ...
                  <div className="bg-slate-950 p-3.5 rounded-2xl space-y-1.5 text-xs text-slate-300 border border-slate-800">
                    ...
                    <div><strong className="text-slate-400">Assigned Operator:</strong> {bus.driver_name || 'Unassigned'}</div>
                  ...
                  <div className="pt-2 border-t border-slate-800 flex items-center justify-between">
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                        bus.status === 'LIVE' || bus.status === 'ACTIVE'
                          ? 'bg-rose-500/20 text-rose-300 hover:bg-rose-500/30 border border-rose-500/40'
                          : 'bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 border border-emerald-500/40'
                      }`}
                    >
                      {bus.status === 'LIVE' || bus.status === 'ACTIVE' ? 'Decommission Vehicle' : 'Deploy Route Dispatch'}
                    </button>
                  </div>
                </div>
              ))}
